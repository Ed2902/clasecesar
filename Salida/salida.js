...
function agregar() {
    ...
    var id_clienteFK = document.getElementById('id_clienteFK').value;
    var valorPorKilo = document.getElementById('valorPorKilo') ? document.getElementById('valorPorKilo').value : ''; // Condicional por si el campo no está en el form

    // Validar campos obligatorios
    if (id_productoFK === '' || id_usuarioFK === '' || peso === '' || id_clienteFK === '' || nombre === '' || referencia === '' || tipo === '') {
    ...
    $("#peso").val('');
    $("#id_clienteFK").val('');
    if (document.getElementById('valorPorKilo')) {
        $("#valorPorKilo").val('');
    }
    $("#id_productoFK").focus();
...
function cantidad(index) {
    ...
    if (item) {
        ...
        $("#tipo").val(item.tipo);
        $("#peso").val(item.peso);
        $("#id_clienteFK").val(item.id_clienteFK);
        if (document.getElementById('valorPorKilo')) {
...
function prepararDatosParaEnvio(data) {
    return data.map(function(producto) {
        ...
        var productoModificado = {
            ...
            "peso": producto.peso,
            "id_clienteFK": producto.id_clienteFK,
